...
int main(int argc, char **argv) {
  ...
  // exception
    ...
    // 4
      ...
      fprintf(stderr,
              "Error: for the edge (u, v), v must be strictly greater than u "
              "(input)\n");
    }
    // 5
    else if (mode == 5) {
      fprintf(stderr,
              "Error: given graph must not have multiple edge (input)\n");
    }
    // 6
    else if (mode == 6) {
      fprintf(stderr,
              "Error: the number of appearance of vertex x_{i, j} must be at "
              "most 1 (output)\n");
    }
    // 7
    else if (mode == 7) {
