...
def exec(command, seed):
    start = time.time()
    subprocess.call(
        '{} < ./testcase/testcase > ./testcase/result'.format(command), shell=True)
    end = time.time()
    score = int(subprocess.check_output(
        './out/score_evaluator.out ./testcase/testcase ./testcase/result', shell=True))
...
for i in range(0, 1000):
    subprocess.call(
        './out/graph_generator.out ./testcase/testcase {}'.format(i), shell=True)
