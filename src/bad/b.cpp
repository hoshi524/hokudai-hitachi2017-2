...
int main() {
  ...
  {  // solve
    if (V <= KR + 1) {
      for (int i = 0; i < MAX_KV; ++i) BEST[i] = MAX_V - 1;
      --V;
      for (int i = 1; i <= V; ++i) {
        int p = ROW * i + 1, d = i & 1;
        for (int j = 1; j < V; ++j) {
      ...
      double sumTime = 0, oneTime = 0;
      int BESTSCORE = 0;
      while (true) {
        double start = timer.getElapsed();
        int size = 40, vertex;
        while (true) {
              ...
              for (int r = 1; r <= KR; ++r) {
                for (int c = 1; c <= KR; ++c) {
                  ...
                  for (int d : direction) {
                    ...
                    int v = -((ps + ns - set.size()) << 10) -
                            (set.size() << 8) + (get_random() & 0xff);
                    value[X[p]][X[n]] = v;
                    value[X[n]][X[p]] = v;
                  }
                }
        ...
        /**
         * マルチケースより１ケースの焼きなまし時間が多いほうがスコアが良かった
         * 終域の連結部分の多様性が無くて、同じような結果になっていそう
         */
        double time = timer.getElapsed() - start;
        sumTime += time;
        if (oneTime < time) oneTime = time;
        if (sumTime + oneTime > TIME_LIMIT) break;
      }
    }
  }
  {  // output
    ...
    for (int i = 0; i < MAX_KV; ++i) {
      if (BEST[i] < MAX_V - 1)
        P[BEST[i]].push_back((i / ROW - 1) * KR + i % ROW);
    }
    for (int i = 0; i < V; ++i) {
