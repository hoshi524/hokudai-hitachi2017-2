    ...
    XSetEnd:
      ...
      if (V < 12) {
        ...
        bool use[MAX_V];
        {
          ...
          sort(wsort, wsort + vertex,
               [WS](int& a, int& b) { return WS[a] > WS[b]; });
          sort(csort, csort + vertex,
               [CS](int& a, int& b) { return CS[a] > CS[b]; });
          for (int i = 0; i < vertex; ++i) {
            x[csort[i]] = wsort[i];
            rev[wsort[i]] = csort[i];
          }
          ...
          memcpy(_connect, connect, sizeof(connect));
          memcpy(_CS, CS, sizeof(CS));
          for (int i = 0; i < vertex; ++i) {
            int16_t* c = _connect[i];
            for (int j = 0; c[j] != -1; ++j) {
              if (!use[c[j]]) {
                --_CS[i];
                c[j] = c[_CS[i]];
        ...
        while (true) {
          ...
          for (int t = 0; t < 0x100; ++t) {
            for (int i = 0; i < CVS; ++i) {
              ...
              int b = _connect[z][get_random() % _CS[z]];
              if (a == b) continue;
              int pv = P[a] + P[b];
              swap(x[a], x[b]);
              int va = value(a);
